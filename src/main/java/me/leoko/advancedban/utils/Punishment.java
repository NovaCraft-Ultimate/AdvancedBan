    ...
    public void create() {
        ...
        if (Universal.get().isUseMySQL()) {
            if (getType() != PunishmentType.KICK)
                Universal.get().getMysql().executeSatement("INSERT INTO `Punishments` (`name`, `uuid`, `reason`, `operator`, `punishmentType`, `start`, `end`, `calculation`) VALUES ('" + getName() + "', '" + getUuid() + "', '" + getReason() + "', '" + getOperator() + "', '" + getType().name() + "', '" + getStart() + "', '" + getEnd() + "', '" + getCalculation() + "')");
...
//        if(PunishmentManager.get().getPunishments().contains(this))
    ...
    public String getDuration(boolean fromStart) {
        ...
        if (getType().isTemp()) {
            ...
            if (diff > 60 * 60 * 24)
                duraton = MessageManager.getMessage("General.TimeLayoutD", "D", diff / 60 / 60 / 24 + "", "H", diff / 60 / 60 % 24 + "", "M", diff / 60 % 60 + "", "S", diff % 60 + "");
            else if (diff > 60 * 60)
                duraton = MessageManager.getMessage("General.TimeLayoutH", "H", diff / 60 / 60 + "", "M", diff / 60 % 60 + "", "S", diff % 60 + "");
            else if (diff > 60)
                duraton = MessageManager.getMessage("General.TimeLayoutM", "M", diff / 60 + "", "S", diff % 60 + "");
